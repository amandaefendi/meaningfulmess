...
```{r}
library(ggplot2)
p = ggplot(df, aes(class, variance, fill=variance))
p + geom_boxplot(width=0.5) + ggtitle("Boxplot of Variance in Fake and Real Bank Notes")
```

```{r}
library(RColorBrewer)
boxplot(df, ylab='Values', main='Overview of BankNote Dataset', col=c(brewer.pal(5, "Spectral")))
```
...
p + geom_boxplot(width=0.5) + geom_jitter(width = 0.01, colour = 'lightsteelblue4', alpha=0.3) + scale_fill_brewer(palette = "Spectral") + ggtitle("Boxplot of Machine Learning Algorithm's Accuracy")  + theme(legend.position='top')
```
