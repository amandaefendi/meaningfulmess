...
simStationary <- simStationary[-(simStationary$MAPE>30),]
```
...
```{r}
design = simStationary[1:2]
design = as.matrix(design); design
```

```{r}
mape_response = simStationary[3]
mape_response = as.matrix(mape_response); mape_response
```

```{r}
# MAPE ~ phi * theta, data = simStationary
fit = buildKrigingDACE(design, mape_response)
fit
```

NOTES: See lecture slides to interpret the Krigging model output above. Find the coefficients of the model and use it for D-optimality. (next part)
...
# EXTRAS - finding errors

```{r}
n=20
k=2
TestCases=maximinLHS(n, k, dup=1)
 
Testscaled=qunif(TestCases, -1, 1)

RMSE=sqrt(sum((predict(fit, newdata=Testscaled)-yvalue(Testscaled[,1],Testscaled[,2]))^2)/n)
RMSE
```
...
for (i in 1:rep) {
  ...
  # fitmodel
  fit=buildKrigingDACE(designscaled,y)
  # eval
  TestCases=maximinLHS(test, k, dup=1)
  Testscaled=qunif(TestCases, -1, 1)
  Error=sqrt(sum((predict(fit, newdata=Testscaled)-yvalue(Testscaled[,1],Testscaled[,2]))^2)/n)
  RMSE[i]=Error
}
